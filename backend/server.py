...
from fastapi.responses import JSONResponse, Response, Response
...
# Monitoring and Observability API Endpoints (must be defined before router inclusion)
...
async def health_check():
    ...
    try:
        ...
        try:
            ...
            update_system_health("database", True)
        except Exception as e:
            health_status["components"]["database"] = {"status": "unhealthy", "error": str(e)}
            update_system_health("database", False)
        ...
        try:
            ...
            if embedding_service:
                ...
                health_status["components"]["embedding_service"] = {"status": "unavailable"}
        ...
        # Check FAISS service
        try:
            faiss_service = getattr(app.state, "faiss", None)
            if faiss_service:
                health_status["components"]["faiss"] = {"status": "healthy"}
                update_system_health("faiss", True)
            else:
                health_status["components"]["faiss"] = {"status": "unavailable"}
                update_system_health("faiss", False)
        except Exception as e:
            health_status["components"]["faiss"] = {"status": "unhealthy", "error": str(e)}
            update_system_health("faiss", False)
        
        # Check Learning-to-Rank engine
        try:
            learning_engine = getattr(app.state, "learning_engine", None)
            if learning_engine:
                health_status["components"]["learning_engine"] = {"status": "healthy"}
                update_system_health("learning_engine", True)
            else:
                health_status["components"]["learning_engine"] = {"status": "unavailable"}
                update_system_health("learning_engine", False)
        except Exception as e:
            health_status["components"]["learning_engine"] = {"status": "unhealthy", "error": str(e)}
            update_system_health("learning_engine", False)
        ...
        health_status["metrics"] = {
            ...
            "total_interactions": await db.recruiter_interactions.count_documents({})
        }
        ...
        for role in ["admin", "recruiter", "candidate"]:
            ...
            update_active_users(role, count)
        
        return health_status
        
    except Exception as e:
        observability_logger.error(f"Health check failed: {e}")
        return {
            "status": "unhealthy",
            "timestamp": datetime.utcnow().isoformat(),
            "error": str(e)
...
async def get_application_metrics(current_user: TokenData = Depends(require_admin)):
    ...
    try:
        ...
        metrics = {
            ...
            "api_metrics": {
                "requests_last_hour": await db.access_logs.count_documents({
                    "timestamp": {"$gte": last_hour}
                }),
                "unique_users_last_hour": len(await db.access_logs.distinct(
                    "user_id", {"timestamp": {"$gte": last_hour}}
                ))
            },
            "resume_processing": {
                "total_processed": await db.candidates.count_documents({}),
                "processed_last_day": await db.candidates.count_documents({
                    "created_at": {"$gte": last_day}
                }),
                "parsing_methods": await db.candidates.aggregate([
                    {"$group": {
                        "_id": "$parsing_method",
                        "count": {"$sum": 1}
                    }}
            ...
            "search_metrics": {
                "searches_last_hour": await db.search_cache.count_documents({
                    "timestamp": {"$gte": last_hour}
                }),
                "searches_last_day": await db.search_cache.count_documents({
                    "timestamp": {"$gte": last_day}
                })
            },
            "learning_metrics": {
                "total_interactions": await db.recruiter_interactions.count_documents({}),
                "interactions_last_day": await db.recruiter_interactions.count_documents({
                    "timestamp": {"$gte": last_day}
                }),
                "interaction_types": await db.recruiter_interactions.aggregate([
                    {"$group": {
        ...
        observability_logger.error(f"Failed to get application metrics: {e}")
        raise HTTPException(status_code=500, detail="Failed to get metrics")

@api_router.get("/monitoring/slo")
async def get_slo_status(current_user: TokenData = Depends(require_admin)):
    """Get Service Level Objectives status (admin only)"""
    try:
        now = datetime.utcnow()
        last_hour = now - timedelta(hours=1)
        last_day = now - timedelta(days=1)
        
        # SLO 1: API response time < 500ms for 99% of requests
        # This would typically come from Prometheus but we'll simulate with access logs
        total_requests_hour = await db.access_logs.count_documents({
            "timestamp": {"$gte": last_hour}
        })
        
        # SLO 2: Resume processing success rate > 95%
        total_resumes_day = await db.candidates.count_documents({
            "created_at": {"$gte": last_day}
        })
        successful_resumes_day = await db.candidates.count_documents({
            "created_at": {"$gte": last_day},
            "parsing_method": {"$ne": None}
        })
        
        resume_success_rate = (successful_resumes_day / total_resumes_day * 100) if total_resumes_day > 0 else 100
        
        # SLO 3: Search result accuracy (based on interaction data)
        total_interactions = await db.recruiter_interactions.count_documents({
            "timestamp": {"$gte": last_day}
        })
        positive_interactions = await db.recruiter_interactions.count_documents({
            "timestamp": {"$gte": last_day},
            "interaction_type": {"$in": ["shortlist", "application", "interview", "hire"]}
        })
        
        search_accuracy = (positive_interactions / total_interactions * 100) if total_interactions > 0 else 100
        ...
        slo_status = {
            ...
            "slos": {
                "api_response_time": {
                    "target": "99% < 500ms",
                    "current_performance": "N/A (requires Prometheus)",
                    "status": "monitoring",
                    "requests_analyzed": total_requests_hour
                },
                "resume_processing_success": {
                    "target": "> 95%",
                    "current_performance": f"{resume_success_rate:.2f}%",
                    "status": "healthy" if resume_success_rate >= 95 else "at_risk",
                    "total_processed": total_resumes_day,
                    "successful": successful_resumes_day
                },
                "search_accuracy": {
                    "target": "Positive interaction rate tracking",
                    "current_performance": f"{search_accuracy:.2f}%",
                    "status": "monitoring",
        ...
        if resume_success_rate < 95:
            slo_status["overall_status"] = "at_risk"
        
        return slo_status
        
    except Exception as e:
        observability_logger.error(f"Failed to get SLO status: {e}")
        raise HTTPException(status_code=500, detail="Failed to get SLO status")

@api_router.get("/monitoring/dashboard")
async def get_monitoring_dashboard(current_user: TokenData = Depends(require_any_auth)):
    """Get monitoring dashboard data"""
    try:
        # Simplified dashboard for internal use
        now = datetime.utcnow()
        last_hour = now - timedelta(hours=1)
        last_day = now - timedelta(days=1)
        ...
        dashboard_data = {
            ...
            "summary": {
                ...
                "recent_activity": {
                    ...
                    "resumes_processed_today": await db.candidates.count_documents({
                        "created_at": {"$gte": last_day}
                    }),
                    "interactions_today": await db.recruiter_interactions.count_documents({
                        "timestamp": {"$gte": last_day}
                    })
                }
            },
        ...
        observability_logger.error(f"Failed to get dashboard data: {e}")
